...
- Routes: `/`, `/metrics`, `/update-schedule`, `/api/v1/blocklist/check`, `/api/v1/reports/false-positive`, `/api/v1/edge/hit` (internal, called by the Cloudflare Worker), `/docs/`
...
Origin runs in a single region, so the most popular `/generated/v1/*.txt`/`.conf` files are additionally cached at the Cloudflare edge via a Worker (`cloudflare/worker.js`) to speed up downloads for geographically distant users - only a curated subset (`cloudflare/routes.json`) is routed through the Worker, kept under the Workers Free plan's 100k requests/day cap; everything else still goes straight to origin, uncached, exactly as before. Cache HITs served from the edge never reach Express, so the Worker reports them asynchronously to `POST /api/v1/edge/hit` (`www/routes/Edge.js`), which reuses the exact same Redis counters as a normal request (`incrementBlocklistStats` in `www/middleware/other/stats-redis.js`) - metrics stay accurate regardless of where a request was served from. Per-file popularity is tracked in Redis (`stats:filepop:<date>`, 14-day TTL) and used weekly by `scripts/refresh-worker-routes.js` (cron in `www/cron/index.js`) to rebuild the candidate list - it only emails a summary, `wrangler deploy` is manual. `scripts/worker-usage-watchdog.js` monitors real Worker invocation counts (`workersZoneInvocationsAdaptiveGroups`, not zone-wide HTTP traffic) every 3h and auto-removes routes if usage nears the daily cap; recovery only restores half of them once a new UTC day has started *and* usage has dropped back below the warning threshold (a same-day, usage-blind restore previously caused back-to-back emergency removals). Blocklist file cache for the Worker-cached subset is invalidated by `scripts/purge-cloudflare-cache.js` (reads `cloudflare/routes.json`), run as part of `npm run update`.

### Bot filtering (`www/utils/isBotRequest.js`)
Stats exclude bot traffic via a UA regex (`www/utils/isBot.js`) plus an IP-verified good-bots check against a companion service (`tcpdata-server`, separate repo) reachable over WebSocket. The check is never live per-request - `isBotRequest` only reads a local Redis cache (`goodbot:<ip>`, 24h TTL); an unseen IP is queued in `goodbot:pending` and resolved in bulk every 2 min by `scripts/resolve-good-bots.js` (cron in `www/cron/index.js`) via one WS round trip. Unset `GOODBOT_WS_URL`/`GOODBOT_WS_SECRET` disables this entirely (regex-only).
